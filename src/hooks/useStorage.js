...
const useStorage = () => {
    ...
    const removeFilm = async (key, item) => { // Função assíncrona que remove um filme do AsyncStorage por meio de uma chave e dados do filme especifico
        try {
            let films = await getFilm(key)// Pega os filmes existentes e os salva em uma variável

            let myFilms = films.filter((film) => film.id !== item.id) /* Remove o filme especifico do array de filmes por meio do filter, 
    ...
    const updateFilm = async (key, item) => {// Função assíncrona que atualiza um filme do AsyncStorage por meio de uma chave e dados do filme especifico
        try {
            let films = await getFilm(key)// Pega os filmes existentes e os salva em uma variável

            let myFilms = films.map( // Utiliza o map para percorrer cada item do array e substitui o item correspondente pelo novo item
